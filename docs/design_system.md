...
|---|---|---:|---:|---:|---:|---|
| Hero Display | CohereText / Space Grotesk | 96px | 400 | 1.00 | -1.92px | Home page declaration scale. |
| Product Display | CohereText / Space Grotesk | 72px | 400 | 1.00 | -1.44px | Product and research hero headlines. |
| Section Display | Unica77 / Inter | 60px | 400 | 1.00 | -1.2px | Large product-page headings. |
| Section Heading | Unica77 / Inter | 48px | 400 | 1.20 | -0.48px | Split hero and CTA headings. |
| Card Heading | Unica77 / Inter | 32px | 400 | 1.20 | -0.32px | Feature card and list section titles. |
| Feature Heading | Unica77 / Inter | 24px | 400 | 1.30 | 0 | Cards, filters, and article titles. |
| Body Large | Unica77 / Inter | 18px | 400 | 1.40 | 0 | Lead text and larger paragraphs. |
| Body | Unica77 / Inter | 16px | 400 | 1.50 | 0 | Default copy and link text. |
| Button | Unica77 / Inter | 14px | 500 | 1.71 | 0 | Compact CTA labels. |
| Caption | Unica77 / Inter | 14px | 400 | 1.40 | 0 | Metadata and small explanatory text. |
| Mono Label | CohereMono / Space Grotesk / Courier | 14px | 400 | 1.40 | 0.28px | Uppercase technical labels. |
| Micro | Unica77 / Inter | 12px | 400 | 1.40 | 0 | Footer, nav microcopy, and small links. |
...
### Spacing System
Base 8px grid with precise alignment values:
`2px`, `6px`, `8px`, `10px`, `12px`, `16px`, `20px`, `22px`, `24px`, `28px`, `32px`, `36px`, `40px`, `56px`, `60px`, `64px`, and `80px`.
...
### `button-primary`
Near-black or white pill CTA, depending on surface contrast. Uses 14px-16px Unica77/Inter, 12px 24px padding, and a 32px pill radius. This is the primary action style for "Request a demo", "Submit", and hero CTAs.
...
### `agent-console-card`
Dark product mockup panel showing agent names, status chips, integration badges, prompt fields, and generated response cards. Background is near-black (`#17171c`), text is white or muted, and small accent chips use product colors.
...
### `dark-feature-band`
Deep green (`#003c33`) or navy (`#071829`) full-width section used for product capabilities, security claims, and feature breakdowns. Text turns white; cards use darker translucent surfaces, pale borders, and abstract line illustrations.

### `product-card`
Warm stone card (`#eeece7`) used for product/model summaries. Typically 3-column on desktop, with 8px radius, generous padding, a small pill button, a divider line, and checkmark bullet rows.
...
### Do
- Use white canvas as the default surface; introduce deep green or dark navy as full-width product bands.
