...
8. ANTI-COPYING RULE: Use the examples to understand the intended interpretation of each taxonomy label. Do not copy an example's answer merely because the wording looks similar. Classify the actual current conversation using the taxonomy and conversation context.
...
Example 56 [AMBIGUOUS]
Status: AMBIGUOUS
Intents: []
Primary Intent: null
Customer: "hi"
Note: Pure greeting without an actionable support issue. Do not force a business intent.

Example 57 [AMBIGUOUS]
Status: AMBIGUOUS
Intents: []
Primary Intent: null
Customer: "can you help me please?"
Note: Vague request for assistance without specific support details.
...
Primary Intent: DELIVERY_DELAYED
Area: DELIVERY_AND_FULFILLMENT
Customer: "hi, my package is late"
Note: Greeting accompanied by an actionable support issue. Actionable problem takes priority.
